...
class InitializationError(MT5LinuxError):
    """Raised when automatic initialization sequence fails.

    This exception wraps errors from rpyc server startup, MT5 launch,
    or connection establishment. The original error is preserved
    in __cause__ for debugging.

    Example:
        >>> raise InitializationError(
        ...     "Cannot initialize: rpyc server failed. Server not responding."
        ... ) from original_error
    """
...
class MetaTrader5(object):
    """MetaTrader5 API for Linux.

    Provides access to MetaTrader5 trading platform via rpyc connection.

    With auto_connect=True (default), the initialization sequence is:
    1. Instantiation (MetaTrader5()) - stores configuration, no connection
    2. initialize() - starts rpyc server, launches MT5, establishes connection

    With auto_connect=False (legacy), behaves like original:
    1. Instantiation - connects immediately to existing rpyc server
    2. initialize() - executes remote mt5.initialize()

    Attributes:
        _host: rpyc server host address.
        _port: rpyc server port number.
        _auto_connect: Whether to use automatic initialization.
        _lifecycle_manager: LifecycleManager for orchestrating startup.
        __conn: rpyc connection object.

    Example (new auto-connect mode):
        >>> mt5 = MetaTrader5()  # No connection yet
        >>> mt5.initialize()     # Auto-starts server, MT5, connects
        >>> mt5.terminal_info()
        >>> mt5.shutdown()

    Example (legacy mode):
        >>> # Assumes rpyc server is already running
    ...
    def __init__(
        ...
        """Initialize MetaTrader5 instance.

        Args:
            host: rpyc server host address. Default 'localhost'.
            port: rpyc server port number. Default 18812.
            auto_connect: If True (default), uses automatic initialization
                sequence in initialize(). If False, connects immediately
                (legacy behavior).

        Note:
            With auto_connect=True, no connection is made until initialize()
            is called. With auto_connect=False, connects immediately to
            an existing rpyc server.
        """
        self._host = host
        self._port = port
        ...
        if auto_connect:
            ...
            logger.debug(
                f"MetaTrader5 instantiated (auto_connect=True): "
                f"host={host}, port={port}"
            )
        else:
            # Legacy behavior - connect immediately
            logger.debug(
                f"MetaTrader5 instantiated (auto_connect=False): "
    ...
    def initialize(self, *args, **kwargs):
        ...
        """
        # Run auto-initialization sequence if auto_connect is enabled
        if self._auto_connect and self.__conn is None:
            self._ensure_initialized()

        code = f"mt5.initialize(*{args},**{kwargs})"
        # rpyc's eval() executes on remote server, not local (noqa: S307)
        return self.__conn.eval(code)  # noqa: S307

    def _ensure_initialized(self) -> None:
        """Run the automatic initialization sequence.

        Sequence:
        1. ensure_server_running() - Start rpyc server if not running
        2. ensure_mt5_running() - Start MT5 terminal if not running
        3. ensure_connected() - Establish rpyc connection
        4. Start HeartbeatMonitor for connection health monitoring

        Raises:
            InitializationError: If any step in the sequence fails.
        """
        from mt5linux.connection import ConnectionError as MT5ConnectionError
        ...
        logger.info("Starting automatic initialization sequence")

        # Create lifecycle manager on first call
        if self._lifecycle_manager is None:
            process_manager = ProcessManager()
            self._lifecycle_manager = LifecycleManager(process_manager)

        try:
            # Step 1: Ensure rpyc server is running
            logger.info("Step 1/4: Ensuring rpyc server is running")
            self._lifecycle_manager.ensure_server_running()
            logger.info("rpyc server running")

            # Step 2: Ensure MT5 terminal is running
            logger.info("Step 2/4: Ensuring MT5 terminal is running")
            self._lifecycle_manager.ensure_mt5_running()
            logger.info("MT5 terminal running")

            # Step 3: Establish connection
            logger.info("Step 3/4: Establishing connection")
            self._lifecycle_manager.ensure_connected()
            logger.info("Connection established")

            # Step 4: Get connection reference for API calls
            # These assertions satisfy MyPy type checker - they cannot be None after
            # successful ensure_connected() call
            assert self._lifecycle_manager is not None
            assert self._lifecycle_manager._connection_manager is not None
            self.__conn = self._lifecycle_manager._connection_manager._conn

            # Step 5: Start heartbeat monitoring
            logger.info("Step 4/4: Starting heartbeat monitoring")
            self._start_heartbeat_monitoring()
            logger.info("Heartbeat monitoring started")

            logger.info("Automatic initialization sequence completed successfully")

        except RpycServerError as e:
            logger.error(f"Failed to start rpyc server: {e}")
            raise InitializationError(
                f"Cannot initialize: rpyc server failed. {e}"
            ) from e
        except MT5LaunchError as e:
            logger.error(f"Failed to launch MT5 terminal: {e}")
            raise InitializationError(
                f"Cannot initialize: MT5 launch failed. {e}"
            ) from e
        except MT5ConnectionError as e:
            logger.error(f"Failed to establish connection: {e}")
            raise InitializationError(
                f"Cannot initialize: connection failed. {e}"
            ) from e
    ...
    def shutdown(self, *args, **kwargs):
        ...
        if self._lifecycle_manager is not None:
            if self._lifecycle_manager._heartbeat_monitor is not None:
                logger.debug("Stopping heartbeat monitoring")
                self._lifecycle_manager._heartbeat_monitor.stop()
                logger.info("Heartbeat monitoring stopped")

            # Disconnect connection manager
            if self._lifecycle_manager._connection_manager is not None:
                logger.debug("Disconnecting connection manager")
                self._lifecycle_manager._connection_manager.disconnect()
                logger.info("Connection manager disconnected")

        # Execute remote shutdown
        code = f"mt5.shutdown(*{args},**{kwargs})"
        # rpyc's eval() executes on remote server, not local (noqa: S307)
        result = self.__conn.eval(code)  # noqa: S307
    ...
    def is_connected(self) -> bool:
        """Check if currently connected to rpyc server.
        ...
        with self._lock:
            if self._auto_connect:
                # Use connection manager's is_connected property
                if self._lifecycle_manager is None:
                    return False
                if self._lifecycle_manager._connection_manager is None:
                    return False
                return self._lifecycle_manager._connection_manager.is_connected
            else:
                # Legacy mode - check if connection exists
    ...
    def connection_status(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            if self._auto_connect and self._lifecycle_manager is not None:
                status["lifecycle_status"] = self._lifecycle_manager.get_status()

                if self._lifecycle_manager._connection_manager is not None:
                    status["connection_info"] = (
                        self._lifecycle_manager._connection_manager.connection_info
                    )
                else:
                    status["connection_info"] = {"connected": False}

                if self._lifecycle_manager._heartbeat_monitor is not None:
                    status["heartbeat_status"] = (
                        self._lifecycle_manager._heartbeat_monitor.get_status()
                    )
                else:
