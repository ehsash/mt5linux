...
class TestConfigureYdotool:
    """Tests for ydotool configuration."""

    def test_configure_ydotool_already_found(self) -> None:
        """Test ydotool configuration when already detected."""
        detection_result = DetectionResult(
            ...
            ydotool=ComponentInfo(found=True, path="/usr/bin/ydotool", version="1.0.0"),
        )
        result = configure_ydotool(detection_result)
        assert result.success is True
        assert result.ydotool_configured is True
        assert result.ydotool_path == "/usr/bin/ydotool"
        assert result.fallback_to_x11 is False

    @patch("mt5linux.setup.wayland_config.detect_ydotool")
    def test_configure_ydotool_detected_on_retry(self, mock_detect: MagicMock) -> None:
        """Test ydotool configuration when detected on retry."""
        mock_detect.return_value = ComponentInfo(found=True, path="/usr/bin/ydotool", version="1.0.0")
        detection_result = DetectionResult(
            environment_type="local",
            display_system="wayland",
            wine=ComponentInfo(found=True),
    ...
    @patch("mt5linux.setup.wayland_config.detect_ydotool")
    def test_configure_ydotool_not_found(self, mock_detect: MagicMock) -> None:
        """Test ydotool configuration when not found."""
        ...
        assert result.ydotool_configured is False
        assert result.fallback_to_x11 is True
        assert result.error == "ydotool not found"


class TestVerifyYdotool:
    """Tests for ydotool verification."""

    @patch("subprocess.run")
    @patch("os.access")
    @patch("os.path.exists")
    def test_verify_ydotool_success(self, mock_exists: MagicMock, mock_access: MagicMock, mock_run: MagicMock) -> None:
        """Test ydotool verification when successful."""
        mock_exists.return_value = True
        mock_access.return_value = True
        mock_run.return_value = MagicMock(returncode=0, stdout="ydotool 1.0.0")
        result = verify_ydotool("/usr/bin/ydotool")
        assert result is True

    @patch("subprocess.run")
    @patch("os.access")
    @patch("os.path.exists")
    def test_verify_ydotool_alternative_version_flag(
        self, mock_exists: MagicMock, mock_access: MagicMock, mock_run: MagicMock
    ) -> None:
        """Test ydotool verification with alternative version flag."""
        mock_exists.return_value = True
        mock_access.return_value = True
    ...
    @patch("os.path.exists")
    def test_verify_ydotool_path_not_exists(self, mock_exists: MagicMock) -> None:
        """Test ydotool verification when path doesn't exist."""
    ...
    @patch("os.access")
    @patch("os.path.exists")
    def test_verify_ydotool_not_executable(self, mock_exists: MagicMock, mock_access: MagicMock) -> None:
        """Test ydotool verification when not executable."""
        mock_exists.return_value = True
...
class TestConfigureWaylandSupport:
    ...
    @patch("mt5linux.setup.wayland_config.verify_ydotool")
    @patch("mt5linux.setup.wayland_config.configure_ydotool")
    def test_configure_wayland_support_success(
        self, mock_configure: MagicMock, mock_verify: MagicMock
    ) -> None:
        """Test Wayland configuration when ydotool is successfully configured."""
    ...
    def test_configure_wayland_support_both_fail(
        ...
        """Test Wayland configuration when both ydotool and X11 fallback fail."""
        mock_configure.return_value = WaylandConfigResult(
            success=False,
            ydotool_configured=False,
            fallback_to_x11=True,
        ...
        result = configure_wayland_support(detection_result)
        assert result.success is False
        assert result.error == "Both ydotool and X11 fallback failed"
