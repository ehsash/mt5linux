    ...
    def test_load_config_creates_defaults_when_file_missing(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch("mt5linux.config._get_config_path", return_value=config_path):
                reload_config()
                config = get_config()
                assert config.wine.prefix_path is None or isinstance(config.wine.prefix_path, str)
...
class TestConfigIntegration:
    ...
    def test_extract_wine_prefix_from_detection_result(self) -> None:
        ...
        detection_result = DetectionResult(
            ...
            mt5=ComponentInfo(
                found=True,
                path=os.path.expanduser("~/.wine/drive_c/Program Files/MetaTrader 5/terminal64.exe"),
            ),
    ...
    def test_config_loading_integration_with_setup(self, mock_detect: MagicMock, mock_get_config: MagicMock) -> None:
        ...
        mock_detect.return_value = DetectionResult(
            ...
            python_system=ComponentInfo(found=True),
            python_windows=ComponentInfo(found=True),
            mt5=ComponentInfo(found=True, path="/saved/wine/prefix/drive_c/Program Files/MetaTrader 5/terminal64.exe"),
            rpyc=ComponentInfo(found=True),
        )
